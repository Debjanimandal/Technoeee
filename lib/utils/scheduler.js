...
export function generateDynamicSchedule(enrollments, pace, startDateStr, completedTasksList, lockedTasksMap = {}) {
  const scheduleMap = {};
  const coursesQueues = [];
  ...
  while (coursesQueues.some(q => q.length > 0) && safety < 365) {
    ...
    // Inject locked tasks (e.g. tasks already completed today)
    if (lockedTasksMap[dateStr]) {
      lockedTasksMap[dateStr].forEach(t => {
        scheduleMap[dateStr].push({
          ...t,
          is_completed: completedTasksList.includes(t.id) || t.is_completed
        });
        slotsUsed++;
      });
    }
    ...
    for (let i = 0; i < coursesQueues.length; i++) {
      ...
      if (queue.length > 0) {
        ...
        // Safely fallback if slotsUsed exceeds TIME_SLOTS (e.g. locked tasks > tasksPerDay)
        const timeSlot = TIME_SLOTS[pace][slotsUsed] || '08:00 PM - 09:30 PM';
